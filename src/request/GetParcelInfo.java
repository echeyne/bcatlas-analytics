/*
 * return the info related to each parcel give a list of parcels
 */
...
public class GetParcelInfo extends HttpServlet {
    ...
    public void retrieveParcelInfo(HttpServletRequest request, HttpServletResponse response) throws ServletException, java.io.IOException {
        ...
        String sql = "SELECT jur_roll, civic_addr, land_size, lot_dim, land_assess, build_assess, total_assess FROM Parcel WHERE jur_roll = ANY (?)";
            ...
            while (rs.next()) {
                builder.add(
                        String.valueOf(rs.getLong("jur_roll")), Json.createObjectBuilder()
                            ...
                            .add("land_assess", ((rs.getString("land_assess") == null) ? "" : rs.getString("land_assess").trim()))
                            .add("build_assess", ((rs.getString("build_assess") == null) ? "" : rs.getString("build_assess").trim()))
                            .add("total_assess", ((rs.getString("total_assess") == null) ? "" : rs.getString("total_assess").trim()))
                );
            }
        } catch (SQLException e) {
            e.printStackTrace();
